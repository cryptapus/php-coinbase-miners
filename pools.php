<?php
/*

List of pool search strings and links. Please add new entries at the top. A 
pool can have multiple 'cbstrings' as to catch different signatures.

*/
return [
    [
        'name' => 'CoinStop',
        'url'  => 'https://coinstop.me',
        'cbstrings' => ['CoinStop.me'],
    ],
    [
        'name' => '1THash.top',
        'url'  => 'https://www.1thash.top/top/index',
        'cbstrings' => ['1THash'],
    ],
    [
        'name' => 'p2p-spb.xyz',
        'url'  => 'http://p2p-spb.xyz:6002/static/',
        'cbstrings' => ['p2p-spb.xyz'],
    ],
    [
        'name' => 'WarpHash',
        'url'  => 'https://warphash.com/',
        'cbstrings' => ['WarpHash'],
    ],
    [
        'name' => 'p2pool StPburg',
        'url'  => 'http://p2p-spb.xyz:5213/static/',
        'cbstrings' => ['/p2p-spb.xyz'],
    ],
    [
        'name' => 'BitHash',
        'url'  => 'https://bit-hash.cloud/',
        'cbstrings' => ['/1THash&58COIN/'],
    ],
    [
        'name' => 'thepool.life',
        'url'  => 'https://thepool.life/',
        'cbstrings' => ['thepool.life'],
    ],
    [
        'name' => 'BTC.com',
        'url'  => 'https://pool.btc.com/',
        'cbstrings' => ['BTC.COM'],
    ],
    [
        'name' => 'ahashpool',
        'url'  => 'https://www.ahashpool.com/',
        'cbstrings' => ['ahashpool'],
    ],
    [
        'name' => 'prohashing.com',
        'url'  => 'https://prohashing.com/',
        'cbstrings' => ['prohashing.com'],
    ],
    [
        'name' => 'blazepool',
        'url'  => 'http://blazepool.com',
        'cbstrings' => ['blazepool'],
    ],
    [
        'name' => 'F2Pool Labs',
        'url'  => 'https://labs.f2pool.com/labs',
        'cbstrings' => ['七彩神仙鱼'],
    ],
    [
        'name' => 'Huobi Pool',
        'url'  => 'https://www.poolhb.com/pow',
        'cbstrings' => ['/Huobi/'],
    ],
    [
        'name' => 'LitecoinPool.org',
        'url'  => 'https://www.litecoinpool.org/',
        'cbstrings' => ['/LP/'],
    ],
    [
        'name' => 'zpool',
        'url'  => 'http://www.zpool.ca/',
        'cbstrings' => ['zpool.ca','zpool'],
    ],
    [
        'name' => 'minersPoolEU',
        'url'  => 'https://www.miners-pool.eu',
        'cbstrings' => ['minersPoolEU'],
    ],
    [
        'name' => 'miningpoolhub',
        'url'  => 'https://miningpoolhub.com/',
        'cbstrings' => ['mph'],
    ],
    [
        'name' => 'myriadcoins.tk',
        'url'  => 'http://myriadcoins.tk/',
        'cbstrings' => ['myriadcoinsTK'],
    ],
    [
        'name' => 'pokemongomongo.tk',
        'url'  => 'http://pokemongomongo.tk/',
        'cbstrings' => ['pokemongomongo'],
    ],
    [
        'name' => 'pool.unitus.online',
        'url'  => 'http://pool.unitus.online',
        'cbstrings' => ['p2poolnz'],
    ],
    [
        'name' => 'EasyMine.online',
        'url'  => 'https://myriad.easymine.online/',
        'cbstrings' => ['EasyMine'],
    ],
    [
        'name' => 'zergpool',
        'url'  => 'https://zergpool.com/',
        'cbstrings' => ['zerg'],
    ],
    [
        'name' => 'hashrefinery',
        'url'  => 'https://pool.hashrefinery.com',
        'cbstrings' => ['hashrefinery'],
    ],
    [
        'name' => 'nlpool.nl',
        'url'  => 'https://nlpool.nl/',
        'cbstrings' => ['NLPOOL', 'NLPool'],
    ],
    [
        'name' => 'mining-dutch.nl',
        'url'  => 'https://www.mining-dutch.nl/',
        'cbstrings' => ['/mining-dutch/', '/Mining-Dutch'],
    ],
    [
        'name' => 'Easy2Mine',
        'url'  => 'https://easy2mine.com/',
        'cbstrings' => ['/E2M & BTC.TOP/'],
    ],
    [
        'name' => 'btc.top',
        'url'  => 'http://btc.top/',
        'cbstrings' => ['/BTC.TOP/'],
    ],
    [
        'name' => 'multipool.us',
        'url'  => 'https://www.multipool.us/',
        'cbstrings' => ['/Multipool/','/Multipool.us/'],
    ],
    [
        'name' => 'ltc.top',
        'url'  => 'http://ltc.top/',
        'cbstrings' => ['LTC.TOP'],
    ],
];
